...
COMMIT=$(git log --pretty="%H" --grep="\[PISCES\] add merge script")
...
# rebase master to Swarm
git remote update swarm > /dev/null
...
git reset --hard swarm/master > /dev/null

echo "Rewrite new marker commit"
git add hack/*
git commit -q -m "[PISCES] add merge script"
